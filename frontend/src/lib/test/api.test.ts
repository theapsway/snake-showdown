  ...
  describe('authApi', () => {
    ...
    describe('signup', () => {
      ...
      it('should signup successfully with new credentials', async () => {
        const result = await authApi.signup('NewPlayer', 'newplayer@test.com', 'password123');
        
        expect(result.success).toBe(true);
        expect(result.user).toBeDefined();
        expect(result.user?.username).toBe('NewPlayer');
        expect(result.user?.email).toBe('newplayer@test.com');
      });
    });
  ...
  describe('leaderboardApi', () => {
    ...
    describe('submitScore', () => {
      ...
      it('should submit score when logged in', async () => {
        ...
        expect(result.entry?.gameMode).toBe('walls');
      });
    });
  });
